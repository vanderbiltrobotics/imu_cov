...
//  prgrammed the accelerometer. I've tried to specialize these for the
...
void ADXL345::init(int16_t address) {
...
void ADXL345::writeTo(byte address, byte val) {
  ...
  rc_i2c_set_device_address(1, (uint8_t) _dev_address);
  rc_i2c_write_byte(1, (uint8_t) address, (uint8_t) val);   // send register address and write val
}

// Reads num bytes starting from address register on device in to _buff array
// If num is < 0, this function does nothing.
// --Josh
void ADXL345::readFrom(byte address, int16_t num, byte _buff[]) {
  ...
  rc_i2c_read_bytes(1, address, (uint8_t) num, _buff);
}
...
void ADXL345::setTapThreshold(int16_t tapThreshold) {
  tapThreshold = (tapThreshold > 0)? tapThreshold : 0;
  tapThreshold = (tapThreshold < 255)? tapThreshold : 255;
...
void ADXL345::setTapDuration(int16_t tapDuration) {
  //tapDuration = (int) std::min(std::max(0, tapDuration), 255);
  tapDuration = (tapDuration > 0)? tapDuration : 0;
  tapDuration = (tapDuration < 255)? tapDuration : 255;
...
void ADXL345::setDoubleTapLatency(int16_t floatTapLatency) {
...
void ADXL345::setDoubleTapWindow(int16_t floatTapWindow) {
  //floatTapWindow = (int) std::min(std::max(0, floatTapWindow), 255);
  floatTapWindow = (floatTapWindow > 0)? floatTapWindow : 0;
  floatTapWindow = (floatTapWindow < 255)? floatTapWindow : 0;
...
void ADXL345::setActivityThreshold(int16_t activityThreshold) {
  //activityThreshold = (int) std::min(std::max(0, activityThreshold), 255);
  activityThreshold = (activityThreshold > 0)? activityThreshold : 0;
  activityThreshold = (activityThreshold < 255)? activityThreshold : 255;
...
void ADXL345::setInactivityThreshold(int16_t inactivityThreshold) {
  //inactivityThreshold = (int) std::min(std::max(0, inactivityThreshold), 255);
  inactivityThreshold = (inactivityThreshold > 0)? inactivityThreshold : 0;
  inactivityThreshold = (inactivityThreshold < 255)? inactivityThreshold : 255;
...
void ADXL345::setTimeInactivity(int16_t timeInactivity) {
  //timeInactivity = (int) std::min(std::max(0, timeInactivity), 255);
  timeInactivity = (timeInactivity > 0)? timeInactivity : 0;
  timeInactivity = (timeInactivity < 255)? timeInactivity : 255;
...
void ADXL345::setFreeFallThreshold(int16_t freeFallThreshold) {
  //freeFallThreshold = (int) std::min(std::max(0, freeFallThreshold), 255);
  freeFallThreshold = (freeFallThreshold > 0)? freeFallThreshold : 0;
  freeFallThreshold = (freeFallThreshold < 255)? freeFallThreshold : 255;
...
void ADXL345::setFreeFallDuration(int16_t freeFallDuration) {
  //freeFallDuration = (int) std::min(std::max(0, freeFallDuration), 255);
  freeFallDuration = (freeFallDuration > 0)? freeFallDuration : 0;
  freeFallDuration = (freeFallDuration < 255)? freeFallDuration : 255;
...
// This is code specifically for the Arduino and need not be used anyway.
//
