...
void FreeSixIMU::init(int16_t acc_addr, int16_t gyro_addr, bool fastmode) {
  usleep(5000);

  // Arduino setup (not necessary in any regard)
  /*
...
void FreeSixIMU::getRawValues(int16_t * raw_values) {
...
void FreeSixIMU::AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
  ...
  float halfex = 0.0, halfey = 0.0, halfez = 0.0;
  float qa, qb, qc;
...
void FreeSixIMU::getQ(float * q) {
  ...
  */
  ...
  // TODO: Not sure if grabbing the current time in micros is going to work!
  // :/
